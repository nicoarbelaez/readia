...
export async function generateSharedObject<T extends z.ZodTypeAny>({
  ...
  for (let i = 1; i <= 2; i++) {
    try {
      console.log(`[ai-helper] Attempt ${i} with Gemini (${modelName})...`);
      const { object } = await generateObject({
        model: googleModel(modelName),
        schema,
        output,
        prompt,
        system,
      ...
      attemptsLog.push({ model: modelName, attempt: i, success: true });
  ...
  // 2. Try OpenRouter
  const openRouterModelName = "openrouter/free";
  const openrouterApiKey = Deno.env.get("OPENROUTER_API_KEY") || "";
  if (!openrouterApiKey) {
    console.warn(
      "[ai-helper] OPENROUTER_API_KEY is not defined. Fallback to OpenRouter might fail.",
    );
  }
  ...
  for (let i = 1; i <= 2; i++) {
    try {
      console.log(
        `[ai-helper] Fallback: Attempt ${i} with OpenRouter (${openRouterModelName})...`,
      );
      const { object } = await generateObject({
        model: openrouterModel(openRouterModelName),
        schema,
        output,
        prompt,
        system,
  ...
  throw new Error(
    `AI generation failed after all attempts. Last error: ${(lastError as any)?.message || lastError}`,
  );
}
